...
app.get('*', (req, res) => {
  ...
  // Last resort
  return res.status(200).send('<h1>Ulric-X MD</h1><p>Bot is running. Web panel files not found.</p><p>Pair API: POST /api/pair</p>');
});

// ─── Start ───────────────────────────────────────────────────────
function startServer(port) {
  // Railway sets PORT env var (usually 8080)
  // Fall back to config.PORT, then 3000
  const finalPort = port || process.env.PORT || config.PORT || 3000;
  console.log(chalk.green(`[WEB] Starting on port ${finalPort} (env PORT=${process.env.PORT || 'not set'})`));
  return app.listen(finalPort, '0.0.0.0', () => {
    console.log(chalk.green(`[WEB] ✅ Panel running on http://0.0.0.0:${finalPort}`));
  });
}
